        ...
        <div class="weui-cell__bd">
            <div class="weui-uploader">
                ...
                <div class="weui-uploader__bd">
                    <div>
                        <gallery ref="gallery" @delAction="delFile"></gallery>
                    </div>
                    <ul class="weui-uploader__files" id="uploaderFiles">
                        <li @click="preview(item, index)" class="weui-uploader__file" :style="{backgroundImage:'url('+item+')'}" v-for="(item, index) in previews" :key="index"></li>
                    </ul>
                    <div class="weui-uploader__input-box" v-if="canUpload">
                        <input id="uploaderInput" class="weui-uploader__input" type="file" accept="image/*" multiple="" @change="selectFile" :disabled="!canUpload">
                    </div>
                </div>
            </div>
        </div>
    ...
    methods: {
        ...
        upload (file) {
            ...
            this.md5(file, (err, md5)=>{
                // 请求配置
                axios.post('api/v1/upload/policy', {filename: file.name, md5: md5}).then(({data}) => {
                    ...
                    axios.post(config.server, param, {loading: false}).then((response) => {
                        this.$loading.hide();
